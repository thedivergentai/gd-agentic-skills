...
`project-foundations`, `characterbody-2d`, `combat-system`, `rpg-stats`, `inventory-system`, `ability-system`, `quest-system`, `economy-system`, `save-load-systems`
...
```
AutoLoads:
├── PlayerStats (rpg-stats)
├── InventoryManager (inventory-system)
├── QuestManager (quest-system)
├── LootGenerator (economy-system)
└── GameManager (scene-management)
